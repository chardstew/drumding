...
class DrumSequencer:
    def __init__(self, root):
        ...
        # ── Start sequencer ─────────────────────────────
        self.running = True
        threading.Thread(target=self.run_sequence_safe, daemon=True).start()

    def run_sequence_safe(self):
        while self.running:
            bpm   = self.bpm_var.get()
            delay = 60.0 / max(1, bpm) / 4.0

            for section in (0,1):
                length = len(self.instruments[0].step_buttons[section])
                for step in range(length):
                    if not self.running:
                        return        # ← immediate escape
                    for inst in self.instruments:
                        note = inst.get_midi_note()
                        if note is None: continue
                        row = inst.step_buttons[section]
                        if step < len(row) and row[step]['bg']==COLORS['on']:
                            # fire and forget
                    ...
                    if not self.running:
                        return        # ← catch it before sleeping
                    time.sleep(delay)

            # ── Row 1 (next up to 32 steps) ─────────────────
            r1_len = len(self.instruments[0].step_buttons[1])
            for step in range(r1_len):
                for inst in self.instruments:
                    note = inst.get_midi_note()
                    if note is None:
                        continue
                    row = inst.step_buttons[1]
                    if step < len(row) and row[step]['bg'] == COLORS['on']:
                        self.midi_out.send(mido.Message('note_on',  note=note, velocity=100))
                        time.sleep(0.01)
                        self.midi_out.send(mido.Message('note_off', note=note, velocity=100))
